...
public class GoogleLoginResponseCommandHandler : ICommandHandler<GoogleLoginResponseCommand, JwtTokenDto>
{
	...
	public async Task<JwtTokenDto> Handle(GoogleLoginResponseCommand request, CancellationToken cancellationToken)
	{
		...
		if (user is null) throw new UserNotFoundException(email ?? string.Empty);

		UserWithPersonalDataDto userDto = (UserWithPersonalDataDto)user;
		var roles = await _userManager.GetRolesAsync(user);
		userDto.Roles = roles;
		if (user.BanDate != null)   //user is banned
		{
			var bannedBy = await _userManager.FindByIdAsync(user.BannedById ?? string.Empty);
			if (userDto.BanInfo != null) userDto.BanInfo.BannedBy = UserGeneralDto.Map(bannedBy);
