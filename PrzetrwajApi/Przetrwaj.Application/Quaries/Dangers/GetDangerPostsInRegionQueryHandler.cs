	...
	: IQueryHandler<GetDangerPostsInRegionQuery, IEnumerable<PostOverviewDto>>
{
	...
	public GetDangerPostsInRegionQueryHandler(IPostRepository postRepository, IRegionRepository regionRepository)
	{
		_postRepository = postRepository;
		_regionRepository = regionRepository;
	}

	public async Task<IEnumerable<PostOverviewDto>> Handle(GetDangerPostsInRegionQuery request, CancellationToken cancellationToken)
	{
		...
		var posts = (await _postRepository.GetDangerByRegionAsync(request.IdRegion, cancellationToken)).ToList();
