...
public partial class PostController : Controller
{
    ...
    [HttpPatch("{id}")]
    [SwaggerOperation("Update post with content. (Owner)")]
    [Authorize]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(ExceptionCasting), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ExceptionCasting), StatusCodes.Status404NotFound)]
    public async Task<IActionResult> UpdatePostById([FromRoute] string id, UpdatePostCommand updatePost, CancellationToken CT)
    {
        if (!ModelState.IsValid) return BadRequest((ExceptionCasting)ModelState);
        try
        {
            await _mediator.Send(new UpdatePostInternalCommand { UpdatePost = updatePost, Id = id }, CT);
            return NoContent();
        }
        catch (BaseException ex)
        {
