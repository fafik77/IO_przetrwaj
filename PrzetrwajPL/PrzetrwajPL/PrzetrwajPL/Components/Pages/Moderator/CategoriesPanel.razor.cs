...
public partial class CategoriesPanel
{
	...
	private string CategoriesPlName => CategoriesType.Equals("dangers", StringComparison.OrdinalIgnoreCase) ? "zagrożeń" : "zasobów";
	...
	private async Task HandleSave()
	{
		...
		try
		{
			...
			{
				successMessage = isNewCategory ? "Pomyślnie dodano nową kategorię." : "Zmiany zostały zapisane pomyślnie.";
			...
			else
			{
				...
				errorMessage = errorResult?.Error?.Message ?? "Wystąpił błąd podczas zapisywania kategorii.";
			}
		}
		catch (Exception ex)
		{
