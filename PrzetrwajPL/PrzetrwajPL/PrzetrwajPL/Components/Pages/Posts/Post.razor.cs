...
public partial class Post
{
    ...
    private const string InitializeSinglePostMapJsName = "initializeSinglePostMap";
    private const string DestroyMapJsName = "destroyMap";
    private const string PostMiniMapId = "postMiniMap";
    private const string PostBigMapId = "postBigMap";
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (!isLoading && post?.LatLong != null && !isMiniMapInitialized)
        {
            isMiniMapInitialized = true;
            await JS.InvokeVoidAsync(InitializeSinglePostMapJsName, PostMiniMapId, post.LatLong.Lat, post.LatLong.Long, post.Title, post.Category?.Name ?? "", false);
        }

        if (showBigMapModal && shouldInitBigMap && post?.LatLong != null)
        {
            shouldInitBigMap = false;
            await Task.Delay(50);
            await JS.InvokeVoidAsync(InitializeSinglePostMapJsName, PostBigMapId, post.LatLong.Lat, post.LatLong.Long, post.Title, post.Category?.Name ?? "", true);
        }
    }
    ...
    {
        showBigMapModal = true;
        shouldInitBigMap = true;
    }

    private async Task CloseBigMap()
    {
        showBigMapModal = false;
        shouldInitBigMap = false;
        try
        {
            await JS.InvokeVoidAsync(DestroyMapJsName, PostBigMapId);
        }
        catch
        {
    ...
    public async ValueTask DisposeAsync()
    {
        try
        {
            await JS.InvokeVoidAsync(DestroyMapJsName, PostMiniMapId);
            await JS.InvokeVoidAsync(DestroyMapJsName, PostBigMapId);
        }
        catch
        {
