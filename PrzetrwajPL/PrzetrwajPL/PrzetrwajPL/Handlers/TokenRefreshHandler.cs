	...
	protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			if (refreshResponse.IsSuccessStatusCode)
			{
				...
				{
					// Update the failing request's header with the fresh Bearer Token
					request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", newTokens.Token);

					// Refresh the local underlying authentication cookie values
					...
					// Lastly: Re-fire the original request using the updated token reference
